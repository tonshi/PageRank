...
public class PageRank {
    ...
    public static class PageRankMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        private static int nums = 25;// peaple数
        ...
        public void map(LongWritable key, Text values, Context context) throws IOException, InterruptedException {
            ...
            if (flag.equals("tmp1")) {
                //String row = values.toString().substring(0,2);
            	String row = tokens[0];
                //String[] vals = PageRankJob.DELIMITER.split(values.toString().substring(2));// 矩阵转置
                String[] vals = PageRankJob.DELIMITER.split(values.toString());// 矩阵转置
                for (int i = 1; i < vals.length; i++) {
                    Text k = new Text(String.valueOf(i));
                    Text v = new Text(String.valueOf("A:" + (row) + "," + vals[i]));
                    context.write(k, v);
                    
                    System.out.println("map: k: " + k.toString() + " v: " + v.toString() + " length: " + vals.length);
                }
